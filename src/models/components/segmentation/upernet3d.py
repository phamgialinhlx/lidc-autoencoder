...
class PyramidPoolingModule(nn.Module):
    ...
    def pool(self, x, size):
        return nn.AdaptiveAvgPool3d(size)(x)
...
class ResidualNet(nn.Module):
    def __init__(self, in_channel, block, layers) -> None:
        ...
        self.stem = nn.Sequential(
            ConvBnAct(in_channel, out_channel=64, kernel=3, stride=2, padding=1, dilation=1),  # /2; c64
            ConvBnAct(64, 64, 1, 1, 0),  # c64
            ConvBnAct(64, 128, 1, 1, 0),  # c128
            nn.MaxPool3d(kernel_size=3, stride=2, padding=1)  # /2; c128
        )
        self.inplanes = 128
...
class UPerNet3D(nn.Module):
    def __init__(self, 
        ...
        self.fpn_dim = fpn_dim
        self.autoencoder_model = load_autoencoder(autoencoder_path, disable_decoder=True, eval=False, map_location="cpu").encoder
        ...
        self.seg = nn.Sequential(ConvBnAct(fpn_dim, fpn_dim, 1, 1, 0), nn.Conv3d(fpn_dim, num_class, 1, 1, 0, bias=True))
        self.out = nn.Conv3d(num_class, num_class, 3, 1, 1)

    def forward(self, x):
        seg_size = x.shape[2:]
        x = self.autoencoder_model(x)
        ...
        list_f.append(F.interpolate(fpn['fpn_layer2'], out_size, mode='trilinear', align_corners=False))
        list_f.append(F.interpolate(fpn['fpn_layer3'], out_size, mode='trilinear', align_corners=False))
        list_f.append(F.interpolate(fpn['fpn_layer4'], out_size, mode='trilinear', align_corners=False))
        x = self.seg(self.fuse(torch.cat(list_f, dim=1)))
        pred = self.out(F.interpolate(x, seg_size, mode='trilinear', align_corners=False))
        return F.softmax(pred, dim=1)

# if __name__ == "__main__":
if __name__ == "__main__":
    img = torch.rand(8, 1, 64, 64, 64).float()
    img = torch.rand(1, 1, 128, 128, 128).float()
    net = UPerNet3D("/mnt/work/Code/lung-thesis/outputs/vq_gan_3d_low_compression/lung-thesis/2aglgm52/checkpoints/epoch=111-step=179200.ckpt")
    out = net(img)
    print(out.shape)