...
class UNet(nn.Module):
    ...
    def forward(self, x):
        x1 = self.autoencoder_model.conv_first(x)
        blocks = self.autoencoder_model.conv_blocks
        x2 = blocks[0].down(x1)
        x2 = blocks[0].res(x2)
        # x2 = self.down1(x1)
        x3 = blocks[1].down(x2)
...
if __name__ == "__main__":
    ...
    unet_model = UNet(
        "/mnt/work/Code/lung-thesis/outputs/vq_gan_3d_low_compression/lung-thesis/2aglgm52/checkpoints/epoch=111-step=179200.ckpt",
        n_channels, 
        n_classes
    )
