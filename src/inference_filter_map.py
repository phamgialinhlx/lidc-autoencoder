...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    model: LightningModule = hydra.utils.instantiate(cfg.model, ckpt_path=ckpt_path).to('cuda')
    ...
    loss, preds, y = model.forward_segmentation(batch)
    encoder = model.encoder
    output = encoder(sample)

    FM_DIR = "swin_cls_seg_soft_dice_v2"
